...
import { EXPORT_NAME_PREFIX, getExportTemplateDir } from "../../config.ts";
...
export async function exportSessionToHtml(
	...
	if (!outputPath) {
		...
		outputPath = `${EXPORT_NAME_PREFIX}-session-${sessionBasename}.html`;
	}
...
export async function exportFromFile(inputPath: string, options?: ExportOptions | string): Promise<string> {
	...
	if (!outputPath) {
		...
		outputPath = `${EXPORT_NAME_PREFIX}-session-${inputBasename}.html`;
	}
