...
const STANDALONE_BINARY_NAME = "mlens";
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-")
		? `${STANDALONE_BINARY_NAME}-${platform}.zip`
		: `${STANDALONE_BINARY_NAME}-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createCliShims(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		for (const command of ["pi", STANDALONE_BINARY_NAME]) {
...
if (!options.skipInstall) {
	...
	createCliShims(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createCliShims(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(
		`  ${join(outDir, `${STANDALONE_BINARY_NAME}-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`,
	);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(
		`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? `${STANDALONE_BINARY_NAME}.exe` : STANDALONE_BINARY_NAME)} --help`,
	);
	...
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? `${STANDALONE_BINARY_NAME}.cmd` : STANDALONE_BINARY_NAME)} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(
			`  ${join(bunInstallDirectory, process.platform === "win32" ? `${STANDALONE_BINARY_NAME}.cmd` : STANDALONE_BINARY_NAME)} --help`,
		);
	}
}
